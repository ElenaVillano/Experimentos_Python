...
total_ensayos=240 #1=24, 2=48, 3=72
...
def instruccionestiempo():
	...
	txt_instruc6 = visual.TextStim(myWindow,text=u"Después de que hayas elegido tu alternativa dando click en la letra, la pantalla te mostrará cuál fue la alternativa que elegiste. Para continuar con la siguiente pregunta tendrás que dar click en el centro de la pantalla. En esta siguiente pregunta se te presentará otro par de alternativas con diferentes cantidades de dinero y demoras y deberás elegir de entre ellas de nuevo.",
		height=1, 
		color=col_alter, colorSpace="rgb255", 
		pos=(0,5),alignHoriz='center',wrapWidth = 40)
...
#Grupo short delays
a_txt = visual.TextStim(myWindow,text= str(a[0])+' pesos en '+str(a[1])+ ' semana',
		height=tamano_letra, 
		color=col_alter,colorSpace='rgb255')
b_txt = visual.TextStim(myWindow,text= str(b[0])+' pesos en '+str(b[1])+ ' semanas',
		height=tamano_letra, 
		color=col_alter, colorSpace="rgb255")
c_txt = visual.TextStim(myWindow,text= str(c[0])+' pesos en '+str(c[1])+ ' semanas',
		height=tamano_letra, 
		color=col_alter, colorSpace="rgb255")
d_txt = visual.TextStim(myWindow,text= str(d[0])+' pesos en '+str(d[1])+ ' semanas',
		height=tamano_letra, 
		color=col_alter, colorSpace="rgb255")
#Grupo long delays
e_txt = visual.TextStim(myWindow,text=str(e[0])+' pesos en '+str(e[1])+' semanas',
		height=tamano_letra, 
		color=col_alter,colorSpace='rgb255')
f_txt = visual.TextStim(myWindow,text=str(f[0])+' pesos en '+str(f[1])+' semanas',
		height=tamano_letra, 
		color=col_alter, colorSpace="rgb255")
g_txt = visual.TextStim(myWindow,text=str(g[0])+' pesos en '+str(g[1])+' semanas',
		height=tamano_letra, 
		color=col_alter, colorSpace="rgb255")
h_txt = visual.TextStim(myWindow,text=str(h[0])+' pesos en '+str(h[1])+' semanas',
		height=tamano_letra, 
		color=col_alter, colorSpace="rgb255")
#Grupo short outcomes
i_txt = visual.TextStim(myWindow,text=str(i[0])+' pesos en '+str(i[1])+' semana',
		height=tamano_letra, 
		color=col_alter,colorSpace='rgb255')
j_txt = visual.TextStim(myWindow,text=str(j[0])+' pesos en '+str(j[1])+' semanas',
		height=tamano_letra, 
		color=col_alter, colorSpace="rgb255")
k_txt = visual.TextStim(myWindow,text=str(k[0])+' pesos en '+str(k[1])+' semanas',
		height=tamano_letra, 
		color=col_alter, colorSpace="rgb255")
l_txt = visual.TextStim(myWindow,text=str(l[0])+' pesos en '+str(l[1])+' semanas',
		height=tamano_letra, 
		color=col_alter, colorSpace="rgb255")

#Conjunto de pares de preguntas, columna por grupo
SetTempo =[[(a_txt,b_txt),(a,b),1],[(b_txt,c_txt),(b,c),2],[(c_txt,d_txt),(c,d),3],[(a_txt,c_txt),(a,c),4],[(b_txt,d_txt),(b,d),5],[(a_txt,d_txt),(a,d),6],
...
SetTempoRep=SetTempo*veces 
SetTempoAle=random.sample(SetTempoRep,len(SetTempoRep))
...
for i in range(len(SetTempoAle)): #ciclo que permite que se repitan los ensayos con diferentes valores
	myMouse=event.Mouse(win=myWindow,newPos=(0,-3))	
	seleccionletra(SetTempoAle[(i)][(0)][(0)],SetTempoAle[(i)][(0)][(1)])
	myWindow.update()
	...
	if y==1: #si y=1 entonces la posicion normal
		u=SetTempoAle[(i)][(1)][(0)],SetTempoAle[(i)][(1)][(1)]
	else: #si y=0 entonces se invierte la posicion
		u=swap(SetTempoAle[(i)][(1)][(0)],SetTempoAle[(i)][(1)][(1)])
...
salvadatos=('sujeto_'+str(sujeto)+'_tiempo.csv')
with open(salvadatos,'wb') as csvfile:
	ensayo=csv.writer(csvfile,delimiter=',')
	ensayo.writerow(['trial','money_left','weeks_left','money_right','weeks_right','choice','biggerchosen','pair','time','date'])
	for i in range(len(SetTempoAle)):
